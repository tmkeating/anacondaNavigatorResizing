...
from anaconda_navigator.api.external_apps import download_utils
...
class HomeTab(WidgetBase):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(self, parent=None):  # pylint: disable=too-many-statements
        ...
        download_utils.DOWNLOADER.instance.sig_configuration.connect(self.update_applications)
        download_utils.DOWNLOADER.instance.sig_resource.connect(self.update_application_icon)
    ...
    def setup(self, conda_data):
        ...
        download_utils.DOWNLOADER.instance.download()
    ...
    def update_applications(self) -> None:
        ...
        telemetry.ANALYTICS.instance.event('filter-applications', {'filter': self.__applications_filter})
    ...
    def update_application_icon(self, resource: str) -> None:
        """Refresh tiles which use `resource` as an icon."""
        item: ListItemApplication
        for item in self.list.items():
            if item.image_path == resource:
                item.set_icon(resource)
