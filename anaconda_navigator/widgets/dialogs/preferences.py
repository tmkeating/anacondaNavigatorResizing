...
from anaconda_navigator.utils import conda_ssl
...
class NavigatorSettingsDialog(SettingsDialog):  # pylint: disable=missing-class-docstring
    ...
    def _save(self, e):
        ...
        if missing_data:
            ...
            self.config.read_string(self.text_edit.toPlainText())
            self.config._save()  # pylint: disable=protected-access
...
class PreferencesDialog(DialogBase):  # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    def create_widget(  # pylint: disable=too-many-arguments,too-many-positional-arguments
            self,
            widget=None,
            label=None,
            option=None,
        ...
        if info:
            ...
            label = PreferencesDialog.update_icon(label, 'info')
    ...
    def create_textbox(  # pylint: disable=too-many-arguments,too-many-positional-arguments
    ...
    def create_checkbox(  # pylint: disable=too-many-arguments,too-many-positional-arguments
            self, label, option, check=None, hint=None, info=None,
    ):
        """Create checkbox preference option."""
        widget = QCheckBox()
        widget.get_value = lambda w=widget: w.checkState() == Qt.Checked
        widget.set_value = lambda value, w=widget: w.setCheckState(Qt.Checked if value else Qt.Unchecked)
    ...
    def create_combobox(  # pylint: disable=too-many-arguments,too-many-positional-arguments
            self, label, option, check=None, hint=None, info=None,
    ):
        widget = ComboBoxBase()

        def set_value(value):
            for i, (prefix, _) in enumerate(self.environments.items()):
                if value == prefix:
    ...
    def setup(self):  # pylint: disable=too-many-statements
        ...
        def ssl_certificate_checker(_: typing.Any) -> tuple[bool, str]:
            """Check if certificate path is valid/exists."""
            ssl_widget = self.widgets_dic.get('ssl_verification')
            ssl_flag: bool = ssl_widget.get_value() if ssl_widget else True
            valid_certificate: bool = conda_ssl.check_cert_option(
                self.widgets_dic.get('ssl_certificate').get_value()) if ssl_flag else True

            return valid_certificate, '' if valid_certificate else 'Invalid certificate!'
...
most web browsers and mobile apps.'''
        ...
        self.create_checkbox('Enable dark mode', 'dark_mode')
        ...
        self.create_checkbox('Hide "what\'s new" dialog on startup', 'hide_whats_new_dialog')
        ...
        self.create_checkbox('Show hidden PSM On-prem channels', 'anaconda_server_show_hidden_channels')
    ...
    def enable_disable_cert(self, value: int | None = None) -> None:
        """Refresh enabled/disabled status of certificate textbox."""
        ssl_cert_widget = self.widgets_dic.get('ssl_certificate')

        if value is None:
            ssl_ver_widget = self.widgets_dic.get('ssl_verification')
            value = ssl_ver_widget.checkState()
        else:
            value = Qt.CheckState(value)

        enabled = value == Qt.CheckState.Checked
        ssl_cert_widget.setEnabled(enabled)

    @staticmethod
    def update_icon(label, state):
        """Update icon for information or warning."""
        label.setProperty('icon', state)
    ...
    def warn(widget, text=None):
        ...
        label = widget.label_information
        if text:
            label = PreferencesDialog.update_icon(label, 'warn')
            ...
            label.setProperty('icon', 'default')
    ...
    def accept(self):
        ...
        if checks and all(checks):
            for widget in self.widgets_changed:
                ...
                if widget.option == 'dark_mode':
                    qapplication().setStyleSheet(styles.load_style_sheet())
